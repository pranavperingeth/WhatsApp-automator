...
const wikitext = `= WhatsApp Contest Automator =
...
Instead of a basic Python script, the automator is built as a highly scalable, production-grade DevOps stack:

* '''Automation Engine:''' [https://n8n.io/ n8n] (Node-based workflow engine)
* '''WhatsApp Gateway:''' [https://evolution-api.com/ Evolution API] (WhatsApp Web Baileys implementation)
* '''Containerization:''' '''Docker''' & <code>docker-compose</code>
* '''Security:''' Full <code>.env</code> variable integration to protect API keys.

----

== 🧠 The "Smart" Notification Logic ==

The system is not a "dumb" cron job. It uses custom JavaScript logic within n8n to provide state-aware memory, ensuring the WhatsApp group is never spammed.
...
* '''The 1-Hour Emergency Siren 🚨:'''
  Uses absolute Unix Epoch time to calculate <code>Contest Start - Current Time</code>. If the difference is exactly <= 75 minutes, it fires the final alarm and locks the memory to prevent duplicate pings.
...
=== 1. Configure Secrets ===
Create a <code>.env</code> file in the root directory to securely inject your credentials:
<pre>
...
=== 3. Connect WhatsApp ===
# Navigate to the Evolution API endpoint: <code>http://localhost:8080/instance/create</code>
# Pass your <code>apikey</code> in the Headers.
# Scan the returned base64 QR code with your WhatsApp app to link the bot.

=== 4. Import the Brain ===
# Open n8n at <code>http://localhost:5678</code>.
# Click '''Import from File''' and upload the <code>workflow.json</code> provided in the repository.
# Open the '''Send WhatsApp''' HTTP node and ensure your Target Group ID and API key are configured.
...
* '''Interactive Commands:''' Upgrade the WhatsApp integration to listen for commands (e.g., <code>@bot next contest</code>) so users can query the schedule on demand.`;
...
async function run() {
    try {
        ...
        console.log("4. Updating page with Wikitext...");
        res = await fetchApi({
            action: 'edit',
            title: TITLE,
            text: wikitext,
            summary: 'Re-formatted document using proper Wikitext syntax',
            token: csrfToken
        }, true);
        ...
        console.log("SUCCESS! Page uploaded with correct formatting.");
        console.log(res);
